...
void setup() {
  ...
  trail = null;
  // create the ostacle
  obstacle1 = new Obstacle(350, 400, 100, 160, #894F3B);
...
void mouseDragged() 
{
  if (trail!=null) 
    ...
    // if trail is drawn through the obstacle delete existing trail
    if(obstacle1.pointInObstacle(mouseX, mouseY))
   trail = new Trail(); 
   
  // if trail is drawn through obstacle print an error message to tell them they have lost
   if (trail!= null) 
  {
    if(obstacle1.pointInObstacle(mouseX, mouseY))
    println("GAME OVER");
